...
const benefits = [
  ...
  { icon: MessageCircleWarning, title: "Détection des Disfluences", description: "Repérage automatique des mots parasites." },
...
const features = [
  ...
  { icon: AudioWaveform, title: "Retour visuel & Disfluences", description: "Visualisez la forme d'onde et la répartition des mots parasites." },
  { icon: BarChart3, title: "Courbes d'évolution", description: "Visualisez l'évolution du SPS moyen sur plusieurs semaines." },
  { icon: Clock, title: "Historique complet", description: "Accédez à toutes les sessions avec enregistrements audio et métriques." },
          ...
          <div className="space-y-3">
            <motion.div key={currentIndex} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.3 }} className="flex items-center gap-3">
              ...
              <div className="flex-1">
                ...
                <div className="text-sm text-muted-foreground">{current.sps.toFixed(1)} syllabes/seconde</div>
              </div>
            </motion.div>
            ...
            <Card className="p-8 md:p-12 border-2 border-primary/20">
              ...
              <div className="grid md:grid-cols-2 gap-10 items-center">
                <div>
                  ...
                  <ul className="space-y-3 mb-8">
                    {["Analyse intelligente : interprétations cliniques auto-générées", "Graphiques intégrés : courbe d'évolution du SPS dans le PDF", "Assiduité documentée : sessions, temps de pratique, série en cours"].map((item, i) => (
                      <li key={i} className="flex items-start gap-3"><CheckCircle2 className="w-5 h-5 text-primary flex-shrink-0 mt-0.5" /><span className="text-muted-foreground">{item}</span></li>
                    ))}
                  </ul>
                  ...
                  <div className="bg-white rounded-xl shadow-lg p-6 transform rotate-1 hover:rotate-0 transition-transform duration-300 border border-border">
                    <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-4 gap-2">
                        {[{ label: "SPS Moyen", value: "4.2" }, { label: "Sessions", value: "12" }, { label: "Assiduité", value: "85%" }, { label: "Tendance", value: "↑" }].map((stat, i) => (
            ...
            <Card className="p-8 md:p-12 border-2 border-success/20 bg-gradient-to-br from-success/[0.02] to-transparent">
              <div className="flex flex-wrap items-center gap-3 mb-6">
                <span className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-success text-success-foreground text-sm font-bold uppercase tracking-wide">
                  <Target className="w-4 h-4" />Biofeedback Temps Réel
                </span>
                <span className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-amber-500/10 text-amber-600 text-sm font-medium">
                  <FlaskConical className="w-4 h-4" />Basé sur Van Zaalen
                </span>
              ...
              <div className="grid md:grid-cols-2 gap-10 items-center">
                <div>
                  ...
                  <p className="text-lg text-muted-foreground mb-6">
                    Le patient sait immédiatement s'il parle trop vite grâce à une jauge colorée intuitive.
                    <strong className="text-foreground"> Plus besoin de chiffres anxiogènes.</strong>
                  </p>
                  <ul className="space-y-3 mb-8">
                    {["Métrique SPS : Syllabes Par Seconde, pas mots/minute — plus précise cliniquement", "Articulatory Rate : calcul excluant les silences", "Feedback émotionnel : emojis et couleurs au lieu de données brutes", "Zone cible personnalisée : le patient voit son objectif en vert"].map((item, i) => (
          ...
          <motion.div className="max-w-5xl mx-auto" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            <Card className="p-8 md:p-12 border-2 border-primary/20 bg-gradient-to-br from-primary/[0.02] to-transparent">
              <div className="flex flex-wrap items-center gap-3 mb-6">
                ...
                <span className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-primary/10 text-primary text-sm font-medium">💬 Transfert en situation réelle</span>
              </div>
              <div className="grid md:grid-cols-2 gap-10 items-center">
                <div>
                  <h2 className="text-3xl md:text-4xl font-bold text-foreground mb-4 leading-tight">
                  ...
                  <ul className="space-y-3 mb-8">
                    {["Transfert des acquis : de l'exercice à la conversation réelle", "Interface épurée : un seul emoji visible de loin, zéro stress", "Sessions courtes : 30 sec à 5 min pour s'intégrer à la séance", "Données traçées : SPS, durée et résultats dans le dossier patient"].map((item, i) => (
                      <li key={i} className="flex items-start gap-3"><CheckCircle2 className="w-5 h-5 text-primary flex-shrink-0 mt-0.5" /><span className="text-muted-foreground">{item}</span></li>
                    ))}
                  </ul>
                  <Button asChild size="lg" className="gap-2"><Link to="/auth">Essayer le Mode Dialogue<ArrowRight className="w-4 h-4 ml-1" /></Link></Button>
                </div>
                <div className="relative">
                  <div className="bg-card rounded-xl shadow-lg p-6 border border-border">
                    <div className="text-center text-sm text-muted-foreground mb-4">Aperçu Mode Dialogue</div>
                    <div className="flex flex-col items-center gap-4">
                      ...
                      <p className="text-xs text-muted-foreground text-center max-w-[200px]">Un seul gros indicateur visible de loin — l'emoji change en temps réel</p>
                      <div className="flex items-center gap-3 w-full mt-2">
                        {[{ emoji: "🐢", label: "Lent", color: "bg-sky-100 dark:bg-sky-900/30" }, { emoji: "✅", label: "Parfait", color: "bg-emerald-100 dark:bg-emerald-900/30 ring-2 ring-primary" }, { emoji: "⚡", label: "Rapide", color: "bg-amber-100 dark:bg-amber-900/30" }].map((state, i) => (
                    ...
                    <div className="text-[10px] text-center text-muted-foreground pt-3 mt-4 border-t border-border">💬 Posez le téléphone et discutez naturellement</div>
                  </div>
                </div>
              </div>
            </Card>
      ...
      <section className="py-20 md:py-28 bg-secondary/30">
        <div className="container px-4 md:px-6">
          <motion.div className="max-w-5xl mx-auto" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            <Card className="p-8 md:p-12 border-2 border-primary/20 bg-gradient-to-br from-primary/[0.02] to-transparent">
              <div className="flex flex-wrap items-center gap-3 mb-6">
                <span className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-primary text-primary-foreground text-sm font-bold uppercase tracking-wide"><BookOpen className="w-4 h-4" />Bibliothèque</span>
                <span className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-primary/10 text-primary text-sm font-medium"><Sparkles className="w-4 h-4" />+90 exercices variés</span>
              </div>
              <div className="grid md:grid-cols-2 gap-10 items-center">
                <div>
                  <h2 className="text-3xl md:text-4xl font-bold text-foreground mb-4 leading-tight">Du contenu <span className="text-primary">pour chaque objectif</span></h2>
                  <p className="text-lg text-muted-foreground mb-6">Une bibliothèque riche et variée pour maintenir l'engagement de vos patients.<strong className="text-foreground"> Fini la monotonie des exercices répétitifs.</strong></p>
                  <ul className="space-y-3 mb-8">
                    {["Exercices progressifs adaptés à chaque niveau", "Catégories ciblées : articulation, souffle, contrôle...", "Nouveaux contenus ajoutés régulièrement"].map((item, i) => (
                      <li key={i} className="flex items-start gap-3"><CheckCircle2 className="w-5 h-5 text-primary flex-shrink-0 mt-0.5" /><span className="text-muted-foreground">{item}</span></li>
                    ))}
                  </ul>
                  <Button asChild size="lg" className="gap-2"><Link to="/auth">Découvrir la bibliothèque<ArrowRight className="w-4 h-4 ml-1" /></Link></Button>
                </div>
                <div className="relative space-y-4">
                  <div className="bg-card rounded-xl shadow-lg p-6 border border-border">
                    <div className="text-center text-sm text-muted-foreground mb-4">Catégories d'exercices</div>
                    <div className="space-y-3">
                      {[
                        ...
                        <div key={i} className="flex items-center gap-3 p-3 bg-muted/50 rounded-lg hover:bg-muted transition-colors">
                          ...
                          <div className="flex-1 flex items-center gap-2">
                            ...
                            {(cat as any).isNew && <span className="px-1.5 py-0.5 text-[10px] font-bold bg-primary text-primary-foreground rounded-full uppercase">Nouveau</span>}
                          </div>
                          {cat.count && <span className="text-xs font-bold text-muted-foreground bg-background px-2 py-1 rounded-full">{cat.count}</span>}
                        </div>
                      ))}
                    </div>
                    <div className="text-[10px] text-center text-muted-foreground pt-3 mt-4 border-t border-border">Contenus créés par des orthophonistes</div>
                  </div>
                  <div className="relative rounded-xl overflow-hidden bg-gradient-to-br from-amber-50 via-yellow-50 to-orange-50 dark:from-amber-950/50 dark:via-yellow-950/40 dark:to-orange-950/40 border-2 border-yellow-200 dark:border-yellow-800 p-5">
                    ...
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-0.5">
                          ...
                          <span className="px-2 py-0.5 text-[10px] font-bold bg-gradient-to-r from-yellow-400 to-orange-400 text-white rounded-full uppercase">Nouveau</span>
                        </div>
                    ...
                    <div className="flex items-center gap-2 text-xl bg-white/60 dark:bg-background/40 rounded-lg px-3 py-2">
                      <span>🏠</span><span className="text-sm text-foreground">→</span><span className="text-sm font-medium text-foreground">"maison"</span>
                      <span className="text-muted-foreground mx-1">·</span>
                      <span>🐱</span><span className="text-sm text-foreground">→</span><span className="text-sm font-medium text-foreground">"chat"</span>
                    </div>
                  </div>
                </div>
              </div>
