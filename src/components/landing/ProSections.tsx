...
import {
  ...
  FileDown, Zap, Dna, CheckCircle2, Flame, Target,
...
const benefits = [
  { icon: Timer, title: "Objective rate measurement", description: "Measures Syllables Per Second excluding pauses." },
  { icon: Dna, title: "Age Calibration", description: "Goals automatically adjusted to each patient's profile." },
...
const features = [
  ...
  { icon: Users, title: "Pediatric Mode", description: "Visual emoji exercises for pre-readers. Breathing bars included." },
];

const ageNorms = [
  { label: "Child", age: "<12 yrs", sps: 4.2, color: "bg-blue-500" },
  { label: "Adolescent", age: "13-20", sps: 5.5, color: "bg-purple-500" },
  { label: "Adult", age: "21-60", sps: 5.0, color: "bg-primary" },
  { label: "Senior", age: ">60 yrs", sps: 4.5, color: "bg-amber-500" },
];

const speedStates = [
  { sps: 3.2, label: "Steady", color: "text-sky-600 dark:text-sky-400", bgColor: "bg-sky-500", dotColor: "bg-sky-500", progress: 40 },
  { sps: 4.1, label: "Perfect", color: "text-success", bgColor: "bg-success", dotColor: "bg-success", progress: 51 },
  { sps: 5.3, label: "Fast", color: "text-warning", bgColor: "bg-warning", dotColor: "bg-warning", progress: 66 },
  { sps: 6.8, label: "Too fast", color: "text-destructive", bgColor: "bg-destructive", dotColor: "bg-destructive", progress: 85 },
];
        ...
        <div className="space-y-5">
          ...
          <div className="space-y-3">
            <motion.div key={currentIndex} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.3 }} className="flex items-center gap-3">
              <span className={`w-8 h-8 rounded-full ${current.dotColor}`} />
          ...
          <div className="grid grid-cols-4 gap-2 pt-3 border-t border-border">
            {speedStates.map((state, i) => (
              <motion.div key={i} className={`text-center p-2 rounded-lg transition-all duration-200 ${i === currentIndex ? "bg-muted ring-2 ring-primary" : "bg-muted/30"}`} animate={{ scale: i === currentIndex ? 1.05 : 1 }}>
                <div className="flex justify-center"><span className={`w-4 h-4 rounded-full ${state.dotColor}`} /></div>
                    ...
                    <div className="space-y-4">
                      {ageNorms.map((group, i) => (
                        <div key={i} className="flex items-center gap-3">
                          <span className={`w-8 h-8 rounded-full ${group.color} flex items-center justify-center text-white text-xs font-bold`}>{group.label.charAt(0)}</span>
                          <div className="flex-1">
            ...
            <Card className="p-8 md:p-12 border border-border">
              ...
              <div className="grid md:grid-cols-2 gap-10 items-center">
                <div>
                  ...
                  <ul className="space-y-3 mb-8">
                    {["SPS metric: Syllables Per Second, not words/minute — more clinically precise", "Articulatory Rate: calculation excluding silences", "Intuitive feedback: colors instead of raw numbers", "Personalized target zone: patient sees their goal in green"].map((item, i) => (
        ...
        <div className="container px-4 md:px-6">
          <motion.div className="max-w-5xl mx-auto" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            <Card className="p-8 md:p-12 border border-border">
              <div className="flex flex-wrap items-center gap-3 mb-6">
                <span className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-primary text-primary-foreground text-sm font-bold uppercase tracking-wide">
                  <MessageSquare className="w-4 h-4" />Dialogue Mode
                </span>
                <span className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-primary/10 text-primary text-sm font-medium"><MessageSquare className="w-4 h-4" /> Transfer to real-life situations</span>
              </div>
              <div className="grid md:grid-cols-2 gap-10 items-center">
                <div>
                  <h2 className="text-3xl md:text-4xl font-bold text-foreground mb-4 leading-tight">
                  ...
                  <ul className="space-y-3 mb-8">
                    {["Skill transfer: from exercise to real conversation", "Clean interface: one large indicator visible from a distance, zero stress", "Short sessions: 30 sec to 5 min to fit into therapy", "Tracked data: SPS, duration and results in patient record"].map((item, i) => (
                      <li key={i} className="flex items-start gap-3"><CheckCircle2 className="w-5 h-5 text-primary flex-shrink-0 mt-0.5" /><span className="text-muted-foreground">{item}</span></li>
                    ))}
                  </ul>
                ...
                <div className="relative">
                  <div className="bg-card rounded-xl shadow-lg p-6 border border-border">
                    ...
                    <div className="flex flex-col items-center gap-4">
                      ...
                      <p className="text-xs text-muted-foreground text-center max-w-[200px]">One large indicator visible from a distance — color changes in real time</p>
                      <div className="flex items-center gap-3 w-full mt-2">
                        {[{ dotColor: "bg-sky-500", label: "Slow", color: "bg-sky-100 dark:bg-sky-900/30" }, { dotColor: "bg-emerald-500", label: "Perfect", color: "bg-emerald-100 dark:bg-emerald-900/30 ring-2 ring-primary" }, { dotColor: "bg-amber-500", label: "Fast", color: "bg-amber-100 dark:bg-amber-900/30" }].map((state, i) => (
                          <div key={i} className={`flex-1 text-center p-2 rounded-lg ${state.color}`}><div className="flex justify-center"><span className={`w-4 h-4 rounded-full ${state.dotColor}`} /></div><div className="text-[10px] text-muted-foreground">{state.label}</div></div>
                        ))}
                      </div>
                    </div>
          ...
          <motion.div className="max-w-5xl mx-auto" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            <Card className="p-8 md:p-12 border border-border">
              <div className="flex flex-wrap items-center gap-3 mb-6">
                ...
                <span className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-primary/10 text-primary text-sm font-medium"><BookOpen className="w-4 h-4" />90+ varied exercises</span>
              </div>
              <div className="grid md:grid-cols-2 gap-10 items-center">
                <div>
                ...
                <div className="relative space-y-4">
                  <div className="bg-card rounded-xl shadow-lg p-6 border border-border">
                    ...
                    <div className="space-y-3">
                      {[
                        ...
                        { icon: MessageSquare, label: "Dialogue Mode", count: null, color: "bg-primary/10 text-primary" },
                      ].map((cat, i) => (
                  ...
                  <div className="relative rounded-xl overflow-hidden bg-muted/50 border border-border p-5">
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      <span className="text-xs font-bold text-muted-foreground bg-background px-2.5 py-1 rounded-full">25 ex.</span>
                    </div>
                    <div className="flex items-center gap-2 text-xl bg-background/60 dark:bg-background/40 rounded-lg px-3 py-2">
      ...
      <section className="py-20 md:py-28">
        <div className="container px-4 md:px-6">
          <motion.div className="max-w-3xl mx-auto text-center" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            ...
            <p className="text-lg text-muted-foreground mb-10 max-w-xl mx-auto">Join the SLPs who use TalkSlower for remote patient monitoring.</p>
